...
#include "fgl/Ast/Relation/NodeEnumToText.h"
#include <fstream>
#include <iostream>
#include <sstream>
#include <vector>
...
int main(int argc, const char* argv[])
{
	...
	for (const auto& filename : filenames)
	{
		...
		auto lexer = fgl::flex::lexer::Lexer();
		auto tokens = lexer.Tokenize(text);
		fgl::bison::parser::Parser();

		std::cout << "Tokens:\n";
		for (auto token : tokens)
		{
			std::cout << "\tTokenName: " << fgl::ast::relation::ConvertEnumToText(static_cast<fgl::ast::Type>(token->Type)) << "\n";
